...
function MachineDetails() {
  ...
  const fetchQueue = async () => {
    try {
      const res = await fetchWithAuth(`/queue/${id}`);
  ...
  useEffect(() => {
    Promise.all([fetchQueue(), fetchMachine()]).finally(() =>
      setLoading(false)
    );
  }, [id]);
  ...
  const handleLeaveQueue = async () => {
    await fetchWithAuth("/queue/leave", {
      method: "POST",
      body: JSON.stringify({ machineId: id, userId: user._id }),
    });
  ...
  const handleStartMachine = async () => {
    const res = await fetchWithAuth("/queue/start", {
      method: "POST",
      body: JSON.stringify({ machineId: id, userId: user._id }),
    });
  ...
  const handleFinishMachine = async () => {
    const res = await fetchWithAuth("/queue/finish", {
      method: "POST",
      body: JSON.stringify({ machineId: id, userId: user._id }),
    });
  ...
  /* Conditions */
  const isUserInQueue =
    user && Array.isArray(queue) &&
    queue.some(q => q?.user?._id === user._id);

  const isUserFirst =
    user && queue.length > 0 &&
    queue[0]?.user?._id === user._id;

  const isUserNext =
    user && queue.length > 1 &&
    queue[1]?.user?._id === user._id;
    ...
    <div className="machines-wrapper">
        ...
        {queue.length === 0 ? (
          ...
          <ul className="queue-list">
            {queue.map((q, i) => {
              ...
              const isRunning =
                i === 0 && machine?.status === "running";
              const isFirstWaiting =
                i === 0 && machine?.status === "available";
              const isNext = i === 1;
              ...
              return (
                ...
                >
                  ...
                  <span>
                    {isRunning && (
                      <span className="queue-tag tag-current">
                        Running
                      </span>
                    )}

                    {isFirstWaiting && (
                      <span className="queue-tag tag-next">
                        First
                      </span>
                    )}
                    ...
                    {isYou && (
                      <span className="queue-tag tag-you">
                        You
                      </span>
                    )}
                  </span>
      ...
      <div style={{ marginTop: 20, display: "flex", gap: 10, justifyContent: "center" }}>
        ...
        {isUserFirst && machine?.status === "available" && (
          <>
            <button className="start-btn" onClick={handleStartMachine}>
              Start Machine
            </button>
        ...
        {isUserFirst && machine?.status === "running" && (
          <button className="finish-btn" onClick={handleFinishMachine}>
            Finish Machine
          </button>
        )}
      </div>
